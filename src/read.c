...
CELL internal_read_list(RCHAN* rchan, int allow_dot, size_t *ret_len);
...
int internal_read_char(RCHAN* rchan)
{
	...
	while(1) {
		...
		char promptbuf[80] = "";
		if (nesting_level > 0) {
			snprintf(promptbuf, sizeof(promptbuf), "%d] %*s", nesting_level, (((nesting_level <= 9) ? nesting_level : 9) - 1) * 3, "");
		}
#ifdef HAVE_READLINE
...
CELL internal_read_number(RCHAN* rchan, int base, int ch, TYPEID want_type)
{
	...
	int seen_dp = 0;
	int seen_exp = 0;
	int got_exp_digits = 0;
	const char *digits =
		(base == 2) ? "01" :
		(base == 8) ? "01234567" :
		(base == 16) ? "01234567890abcdef" : "0123456789";
	...
	int accept = 1;
	while(EOF != ch && i < max_len) {
		if (i == 0 && (ch == '-' || ch == '+')) {
		}
		else if (strchr(digits, ch)) {
			got_exp_digits = seen_exp;
		}
		else if (base != 10) {
			accept = 0;
			break;
		}
		else if (want_type == T_INT) {
			break;
		}
		else if (ch == '.' && !seen_dp && !seen_exp) {
			seen_dp = 1;
		}
		else if (strchr(exponent_markers, ch) && !seen_exp) {
			ch = 'e';
			seen_exp = 1;
		}
		else if (i && token[i-1] == 'e' && (ch == '+' || ch == '-')) {
		}
		else {
			accept = 0;
			break;
	...
	token[i] = '\0';
	if (seen_exp || seen_dp || want_type == T_FLOAT) {
...
CELL internal_read_char_const(RCHAN* rchan)
{
	...
	// only needs to accomodate longest valid named character
		...
		case '#':
			{
				...
				case 'b': return internal_read_number(rchan, 2, -1, T_EMPTY);
				case 'o': return internal_read_number(rchan, 8, -1, T_EMPTY);
				case 'd': return internal_read_number(rchan, 10, -1, T_EMPTY);
				case 'x': return internal_read_number(rchan, 16, -1, T_EMPTY);
				default:
...
CELL internal_read_vector(RCHAN* rchan)
{
	...
	size_t len;
	list = internal_read_list(rchan, 0, &len);
	if (EXCEPTIONP(list)) {
		gc_unroot();
		return list;
	}
...
CELL internal_read(RCHAN* rchan, CELL token)
{
	...
	if (EQP(token, V_CHAR_LPAREN)) { return internal_read_list(rchan, 1, 0); }
	if (EQP(token, V_CHAR_RPAREN)) { return make_exception("unexpected ')'"); }
	if (EQP(token, V_CHAR_DOT)) { return make_exception("illegal use of '.'"); }

	CELL prefix = V_EMPTY;
	if      (EQP(token, V_CHAR_QUOTE))      { prefix = V_QUOTE; }
	else if (EQP(token, V_CHAR_QUASIQUOTE)) { prefix = V_QUASIQUOTE; }
	else if (EQP(token, V_CHAR_COMMA))      { prefix = V_UNQUOTE; }
	else if (EQP(token, V_COMMA_AT))        { prefix = V_UNQUOTE_SPLICING; }
	else { return token; }
...
char * completion_generator(const char* text, int state)
{
    ...
    while(!NULLP(symbol_list)) {
        ...
        if (!UNDEFINEDP(p->binding) && len <= p->len && strncmp(p->data, text, len) == 0) {
